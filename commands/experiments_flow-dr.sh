...
CUDA_VISIBLE_DEVICES=$gpu_id python train.py policy="sac" simba=false\
...
do
    for init_lmbda in 0.01 0.1 1 10
        ...
        CUDA_VISIBLE_DEVICES=$gpu_id python train.py policy="flowsac" delta=0 flow_lr=$flow_lr single_lambda=true \
            ...
            seed=$seed use_wandb=$use_wandb dr_flow=$dr_flow dr_train_ratio=$dr_train_ratio simba=false
        done
done