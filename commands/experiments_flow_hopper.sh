...
CUDA_VISIBLE_DEVICES=$gpu_id python train.py policy="sac" \
                            randomization=false eval_randomization=false wandb_project=$wandb_project use_wandb=$use_wandb seed=$seed\
...
do
    for init_lmbda in 0.00001 0.0001 0.001 0.01
        do
