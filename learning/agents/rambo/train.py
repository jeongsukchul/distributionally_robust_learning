...
def train(
    ...
    rollout_batch_size : int = 100000, #added
...
#   num_training_steps_per_epoch = num_training_steps_per_epoch//model_train_freq * model_train_freq
  ...
  fake_batch_size = batch_size  * grad_updates_per_step // device_count - real_batch_size
  ...
  fake_buffer = DynamicBatchQueue(
      max_replay_size= int(max_replay_size //device_count),
      dummy_data_sample=dummy_transition,
  ...
  def training_step(
    ...
    metrics['fake_buffer_current_size'] = replay_buffer.size(fake_buffer_state)  # pytype: disable=unsupported-operands  # lax-types
  ...
  def dynamics_step(
    ...
    return (training_state.replace(
      dynamics_optimizer_state=dynamics_optimizer_state,
      dynamics_params = dynamics_params,), key),  {'dynamics_loss' : dynamics_loss}
  ...
  def training_model_interval(
    ...
    metrics.update(rollout_info)
        ...
        #         lambda x, y: x + y, accumulated_metrics, metrics
        # )
    metrics= jax.tree_util.tree_map(jnp.mean, metrics)
    # metrics = jax.tree_util.tree_map(lambda x: x / int(num_training_steps_per_epoch//model_train_freq), accumulated_metrics)         
    return training_state, env_state, buffer_state, fake_buffer_state, metrics
  def training_epoch(
      training_state: TrainingState,
      env_state: envs.State,
    ...
    def f(carry, unused_t):
      ts, es, bs, fs, k = carry
      k, new_key = jax.random.split(k)
      ts, es, bs, fs, metrics = training_model_interval(ts, es, bs, fs, k)
      return (ts, es, bs, fs, new_key), metrics
    (training_state, env_state, buffer_state, fake_buffer_state, key), metrics = jax.lax.scan(
        f,
        (training_state, env_state, buffer_state, fake_buffer_state, key),
        (),
        length=num_training_steps_per_epoch//model_train_freq,
    )
    metrics= jax.tree_util.tree_map(jnp.mean, metrics)
    return training_state, env_state, buffer_state, fake_buffer_state, metrics
  ...
  def training_epoch_with_timing(
    ...
    t = time.time()
    (training_state, env_state, buffer_state, fake_buffer_state, metrics) = training_epoch(
        training_state, env_state, buffer_state, fake_buffer_state, key
    )
    metrics = jax.tree_util.tree_map(jnp.mean, metrics)
  ...
  for _ in range(num_evals_after_init):
    ...
    (training_state, env_state, buffer_state, fake_buffer_state, training_metrics) = (
        training_epoch_with_timing(
            training_state, env_state, buffer_state, fake_buffer_state, epoch_keys
        )
    )
