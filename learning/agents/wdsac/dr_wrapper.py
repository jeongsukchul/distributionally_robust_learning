...
class EpisodeWrapper(Wrapper):
  ...
  def reset(self, rng: jax.random.PRNGKey) -> State:
    ...
    state.info['steps'] = jnp.zeros(reset_rng.shape[:-1])
    state.info['truncation'] = jnp.zeros(reset_rng.shape[:-1])
    # Keep separate record of episode done as state.info['done'] can be erased
    # by AutoResetWrapper
    state.info['episode_done'] = jnp.zeros(reset_rng.shape[:-1])
    episode_metrics = dict()
    episode_metrics['sum_reward'] = jnp.zeros(reset_rng.shape[:-1])
    episode_metrics['length'] = jnp.zeros(reset_rng.shape[:-1])
    for metric_name in state.metrics.keys():
      episode_metrics[metric_name] = jnp.zeros(reset_rng.shape[:-1])
...
class BraxAutoResetWrapper(Wrapper):
  ...
  def step(self, state: mjx_env.State, action: jax.Array, rng: jax.random.PRNGKey) -> mjx_env.State:
