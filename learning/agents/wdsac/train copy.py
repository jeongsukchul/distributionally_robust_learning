...
def _init_training_state(
  ...
  normalizer_params = running_statistics.init_state(
      specs.Array((obs_size,), jnp.dtype('float32')) if isinstance(obs_size, int) else obs_size,
  )
...
def train(
  ...
  if wrap_env:
    ...
    env = wrap_for_dr_training(
        ...
        n_envs=num_envs,# // jax.process_count() // local_devices_to_use,
  ...
  dummy_next_obs = { key: jnp.zeros((n_nominals,) + obs_shape[key]) for key in obs_shape } if isinstance(obs_shape, dict) else jnp.zeros((n_nominals, obs_shape))
  print("dummy_obs", dummy_obs)
  print("dummy_next_obs", dummy_next_obs)
  ...
  if distance_type == "kl":
    lmbda_update = gradients.gradient_update_fn(
      kl_lmbda_loss, lmbda_optimizer, has_aux= True, pmap_axis_name=None#_PMAP_AXIS_NAME
    )
  elif distance_type == "tv":
    lmbda_update = gradients.gradient_update_fn(
      tv_lmbda_loss, lmbda_optimizer, has_aux= True, pmap_axis_name=None#_PMAP_AXIS_NAME
    )
  else:
    lmbda_update = gradients.gradient_update_fn(  # pytype: disable=wrong-arg-types  # jax-ndarray
        lmbda_loss, lmbda_optimizer , has_aux=True, pmap_axis_name=None#_PMAP_AXIS_NAME
    )
  alpha_update = gradients.gradient_update_fn(  # pytype: disable=wrong-arg-types  # jax-ndarray
      alpha_loss, alpha_optimizer , pmap_axis_name=None#_PMAP_AXIS_NAME
  )
  critic_update = gradients.gradient_update_fn(  # pytype: disable=wrong-arg-types  # jax-ndarray
      critic_loss, q_optimizer , pmap_axis_name=None#_PMAP_AXIS_NAME
  )
  actor_update = gradients.gradient_update_fn(  # pytype: disable=wrong-arg-types  # jax-ndarray
      actor_loss, policy_optimizer, pmap_axis_name=None#_PMAP_AXIS_NAME
  )
  ...
  def sgd_step(
    ...
    def lambda_optimization_step(carry, unused):
          ...
          return (new_lmbda_params, new_lmbda_optimizer_state, lmbda_loss), (lmbda_loss, next_v, min_indices, loss_info)
      
    (lmbda_params, lmbda_optimizer_state, _), (lmbda_loss, next_v, min_indices, loss_info) = jax.lax.scan(
  ...
  # If local_devices_to_use = 4

  # assert prefill_keys.shape[0] == local_devices_to_use

  training_state, env_state, buffer_state, _ = prefill_replay_buffer(
      training_state, env_state, buffer_state, prefill_key#s
  )
  print("memory used after prefill", process.memory_info().rss / 1024 ** 2, "MB")
  ...
  for _ in range(num_evals_after_init):
    ...
    (training_state, env_state, buffer_state, training_metrics) = (
        training_epoch_with_timing(
            training_state, env_state, buffer_state, epoch_key#s
        )
    )
    print("memory used after training epoch", process.memory_info().rss / 1024 ** 2, "MB")
