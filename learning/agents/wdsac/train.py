...
def _init_training_state(
  ...
  lmbda = jnp.asarray(0.1, dtype=jnp.float32)
  lmbda_optimizer_state = lmbda_optimizer.init(lmbda)
...
#   assert num_envs % device_count == 0
  ...
  lmbda_optimizer = optax.adam(learning_rate=3e-4)
  ...
  lmbda_loss, alpha_loss, critic_loss, actor_loss = wdsac_losses.make_losses(
  ...
  def sgd_step(
    ...
    lmbda = training_state.lmbda_params
    (lmbda_loss, next_v), lmbda_params, lmbda_optimizer_state = lmbda_update(
  ...
  def random_step(
    ...
    env_key, action_key1, action_key2, rollout_key = jax.random.split(key,3)
    actions, policy_extras = policy(env_state.obs, action_key1)
    nstate = env.step(env_state, actions, env_key)
    next_actions = policy(nstate.obs, action_key2)
    state_extras = {x: nstate.info[x] for x in extra_fields}
    idx = jnp.arange(n_nominals*num_envs).reshape(n_nominals, num_envs)


    print("next actions", next_actions)
    idx = jax.random.randint(rollout_key, (num_envs,), 0, n_nominals*num_envs)
    rollout_next_state = jax.tree_util.tree_map(lambda x : x[idx])

    print("next state", rollout_next_state)
...
#   env_state = jax.pmap(env.reset)(env_keys)
  ...
  print("env state episode done", env_state.info["episode_done"])
  # Replay buffer init
