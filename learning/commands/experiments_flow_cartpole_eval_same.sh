gpu_id=$1 
wandb_project="flow-cartpole-eval"
seed=$2
use_wandb=false
...
CUDA_VISIBLE_DEVICES=$gpu_id python train.py policy="flowsac" delta=0 flow_lr=1e-5 single_lambda=true \
            lambda_update_steps=10 lmbda_lr=0. wandb_project=$wandb_project asymmetric_critic=true init_lmbda=0.01 task=$task \
            seed=$seed use_wandb=$use_wandb dr_flow=$dr_flow dr_train_ratio=$dr_train_ratio eval_with_training_env=false
