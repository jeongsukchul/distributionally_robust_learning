gpu_id=$1  # First argument passed to the script
wandb_project="wdsac-wass"  # Define your project name here
...
do
    ...
    do
        CUDA_VISIBLE_DEVICES=$gpu_id python train.py policy="wdsac" n_nominals=$n_nominals delta=$delta single_lambda="False" \
            lambda_update_steps=10 distance_type="wass" wandb_project=$wandb_project init_lmbda=0.1
    done
done
