...
class NormalizingFlow(nn.Module):
    """
    Normalizing Flow model q(x) constructed as pushforward of base q0(z)
    through a sequence of invertible flows.
    Assumptions:
      - Each flow f returns (y, logabsdet) when called as f(x).
      - Each flow has .inverse(x) -> (z, logabsdet_inv).
      - Base distribution q0 provides:
          - forward(num_samples, **kwargs) -> (z, log_q0(z)) and uses rng 'sample'
          - log_prob(z) -> log q0(z)
      - Target distribution p provides:
          - log_prob(x) -> log p(x)
    """
    q0: nn.Module                     # base distribution module
    flows: Tuple[nn.Module, ...]      # tuple of flow modules
    p: Optional[nn.Module] = None     # target distribution (for KLs), optional
    # @nn.compact
    def __call__(self, mode : str,                 
        ...
        if mode=="forward":
            ...
            return self.sample(num_samples, rng)
        elif mode=="log_prob":
    ...
    def _zeros_like_batch(x: jnp.ndarray) -> jnp.ndarray:
        # Assume last dimension is event dimension; keep leading batch dims
        return jnp.zeros(x.shape[:-1], dtype=x.dtype)

    def forward(self, z: jnp.ndarray):
        """Apply all flows in order. Returns (x, sum log|det J_f|)."""
        log_det = self._zeros_like_batch(z)
        x = z
        for flow in self.flows:
            x, ld = flow(x)           # forward of flow
            log_det = log_det + ld
        return x, log_det

    # z = f_1^{-1} ∘ ... ∘ f_K^{-1} (x)
    def inverse(self, x: jnp.ndarray):
        """Apply all inverses in reverse order. Returns (z, sum log|det J_inv|)."""
        log_det = self._zeros_like_batch(x)
        z = x
    ...
    def log_prob(self, x: jnp.ndarray) -> jnp.ndarray:
        """
        Log q(x) via change of variables:
          z, logdet_inv = f^{-1}(x)
          log q(x) = log q0(z) + logdet_inv
        """
        z, logdet_inv = self.inverse(x)
        return self.q0.log_prob(z) + logdet_inv

    def sample(self, num_samples: int, key, **kwargs):
        """
        Sample x ~ q by sampling z ~ q0 and pushing forward.
        Requires rngs={'sample': key} for q0.forward.
        Returns:
          x: (N, *event_shape), log_q(x): (N,)
        """
        z, log_q0 = self.q0.forward(key, num_samples=num_samples, **kwargs)  # (N, D), (N,)
        x, logdet = self.forward(z)
        log_q = log_q0 - logdet
        return x, log_q
    def forward_kld(self, x: jnp.ndarray) -> jnp.ndarray:
        """
        Estimate KL(p || q) using samples x ~ p (provided).
          KL(p||q) = E_p[-log q(x)] + const
        Here we return E_p[-log q(x)] (const wrt q).
        """
    ...
    def reverse_kld(
        ...
        **q0_kwargs,
        ...
        """
        Estimate KL(q || p) given target p:
          E_q[log q(x) - beta * log p(x)]
        Draw x by pushing z ~ q0 through the flow.

        score_fn:
          - True  (default): include gradients through log q(x).
          - False: exclude score-function terms -> stop_gradient(log q(x)).
        """
        if self.p is None:
        ...
        log_p = self.p.log_prob(x)                     # (N,)
        return jnp.mean(log_q) - beta * jnp.mean(log_p)
    ...
    #     if dreg:
    #         w_const = jnp.exp(log_p - log_q).detach()
    #         z_ = z
    #         log_q = jnp.zeros(len(z_), device=z_.device)
...
#         """
#         log_det = jnp.zeros(len(z), device=z.device)
#         for flow in self.flows:
...
#           Jacobian
#         """
#         log_det = jnp.zeros(x.shape[0], device=x.device)
#         for i in range(len(self.flows) - 1, -1, -1):
...
#           log probability
#         """
#         log_q = jnp.zeros(x.shape[0], dtype=x.dtype, device=x.device)
#         z = x
#         for i in range(len(self.flows) - 1, -1, -1):
...
#         """
#         log_q = jnp.zeros(x.shape[0], device=x.device)
#         z = x
#         for i in range(len(self.flows) - 1, -1, -1):
#             z, log_det = self.flows[i].inverse(z, context=context)
#             log_q += log_det
...
#         """
#         log_det = jnp.zeros(len(z[0]), dtype=z[0].dtype, device=z[0].device)
...
#         """
#         log_det = jnp.zeros(x.shape[0], dtype=x.dtype, device=x.device)
#         if self.transform is not None:
