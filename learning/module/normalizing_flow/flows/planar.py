...
class Planar(Flow):
    ...
    def setup(self):
        ...
        if self.u_init is None:
            self.u = self.param('u', _rand_uniform(-lim_u,lim_u), self.shape)
        else:
            self.u = self.param('u', lambda x: self.u_init)
        if self.w_init is None:
            self.w = self.param('w', _rand_uniform(-lim_w,lim_w), self.shape)
        else:
            self.w = self.param('w', lambda x: self.w_init)
    ...
    def forward(self, z) -> Tuple[Array, Array]:
        ...
        z_out = z+ h_lin[...,None] * u_hat[None,...]       #[..., D]
